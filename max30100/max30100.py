""""
  derived from : https://github.com/mfitzp/max30100/blob/master/max30100.py
  adapted for the pyboard
...
class MAX30100(object):
    ...
    def __init__(self,
        ...
        self.pin_int = pyb.Pin('Y8', mode=pyb.Pin.IN, pull=pyb.Pin.PULL_UP)
    ...
    def set_mode(self, mode):
        ...
        # reg = self.i2c.mem_read(1, I2C_ADDRESS, MODE_CONFIG)
        ...
        self.i2c.mem_write((reg & 0xF8) | mode, I2C_ADDRESS, MODE_CONFIG)  # mask the SHDN bit
        # self.i2c.mem_write((reg & 0x74)|mode, I2C_ADDRESS, MODE_CONFIG) # mask the SHDN bit

    def set_spo_config(self, sample_rate=SAMPLE_RATE[100], pulse_width=PULSE_WIDTH[1600]):
        # reg = self.i2c.read_byte_data(I2C_ADDRESS, SPO2_CONFIG)
        # reg = self.i2c.mem_read(1, I2C_ADDRESS, SPO2_CONFIG)
        reg = self.read_register(SPO2_CONFIG)
        # self.i2c.write_byte_data(I2C_ADDRESS, SPO2_CONFIG, reg | pulse_width)
        self.i2c.mem_write((reg & 0xFC) | pulse_width, I2C_ADDRESS, SPO2_CONFIG)
        self.i2c.mem_write((reg & 0xE3) | (sample_rate << 2), I2C_ADDRESS, SPO2_CONFIG)
    ...
    def enable_interrupt(self, interrupt_type):
        ...
        # self.i2c.mem_read(1, I2C_ADDRESS, INT_STATUS)
    ...
    def get_number_of_samples(self):
        ...
        # write_ptr = self.i2c.mem_read(1, I2C_ADDRESS, FIFO_WR_PTR)
        # read_ptr = self.i2c.mem_read(1, I2C_ADDRESS, FIFO_RD_PTR)
    ...
    def get_rev_id(self):
        # return self.i2c.mem_read(I2C_ADDRESS, REV_ID)
        # return self.i2c.mem_read(1, I2C_ADDRESS, REV_ID)
        return self.read_register(REV_ID)

    def get_part_id(self):
        # return self.i2c.read_byte_data(I2C_ADDRESS, PART_ID)
        # return self.i2c.mem_read(1, I2C_ADDRESS, PART_ID)
    ...
    def generator(self, delay=None):
        self.reset()
        self.set_led_current(led_current_red=self.led_current_red,
                             led_current_ir=self.led_current_ir)
        self.set_spo_config(sample_rate=self.sample_rate, pulse_width=self.pulse_width)
        self.set_mode(MODE_HR)
        pyb.delay(200)
        ...
        while True:
            if sample % 400 == 0:
                self.enable_interrupt(INTERRUPT_TEMP)
                self.refresh_temperature()
                while self.pin_int.value():
                    pass  # waitfor temp trigger
                temperature = self.get_temperature()

            self.enable_interrupt(INTERRUPT_HR)
